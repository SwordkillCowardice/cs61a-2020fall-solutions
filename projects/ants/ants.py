...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        if self.exit:  # 任何一个place，只有在被作为出口传入构造时，才会拥有自己的入口
            exit.entrance = self
        # END Problem 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        range1, tmp = 0, self.place
        while not tmp.is_hive:
            if tmp.bees and self.lower_bound <= range1 <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        rest = self.health - amount
        hurt = amount if rest > 0 else (amount + self.damage)
        tmp = list(self.place.bees)
        for bee in tmp:
            Insect.reduce_health(bee, hurt)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown == 0:
            goal = random_bee(self.place.bees)
            if goal:
                Insect.reduce_health(goal, goal.health)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while tmp:
            goal = tmp.ant
            if goal:
                if goal.is_container and goal.ant_contained:
                    goal.ant_contained.double()
                goal.double()
            tmp = tmp.exit       
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def make_slow(bee):  # 使用高阶函数来封装对象本身(即self), 从而实现类似调用实例方法的绑定效果
        def slow_action(gamestate):
            if gamestate.time % 2 == 0:
                Bee.action(bee, gamestate)
            bee.slow_turn -= 1
            if bee.slow_turn == 0:
                bee.action = SlowThrower.recover(bee)
        return slow_action
    
    def recover(bee):
        def normal_action(gamestate):
            Bee.action(bee, gamestate)
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if target:
            target.slow_turn = self.slow_round
            target.action = SlowThrower.make_slow(target)
        # END Problem EC 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem EC 4
        ans = {}
        tmp, distance = self.place, 0
        while not tmp.is_hive:
            if tmp.ant:
                if tmp.ant.is_container:
                    ans[tmp.ant] = distance
                    if tmp.ant.ant_contained and tmp.ant.ant_contained.name != 'Laser':
                        ans[tmp.ant.ant_contained] = distance
                elif tmp.ant.name != 'Laser':
                    ans[tmp.ant] = distance
            for bee in tmp.bees:
                ans[bee] = distance
            tmp = tmp.entrance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        ans = self.damage - distance * 0.25 - 0.0625 * self.insects_shot
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        destination = self.place.exit if not self.scared_round else self.place.entrance
