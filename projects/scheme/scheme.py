# Questions 19-20 were not completed
...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        para = rest.map(eval_single)
        return scheme_apply(proc, para, env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.bindings[func] = do_lambda_form(Pair(paras, body), env)
        return func
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    names, values = Pair(first_binding.first, nil), Pair(scheme_eval(first_binding.rest.first, env), nil)
    names_tail, values_tail = names, values
    bindings = bindings.rest

    while bindings != nil:
        next_binding = bindings.first
        validate_form(next_binding, 2, 2)
        names_tail.rest, values_tail.rest = Pair(next_binding.first, nil), Pair(scheme_eval(next_binding.rest.first, env), nil)
        names_tail, values_tail = names_tail.rest, values_tail.rest
        bindings = bindings.rest
